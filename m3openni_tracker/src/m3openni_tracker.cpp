...
int main(int argc, char **argv) {
    ros::init(argc, argv, "m3openni_tracker");
    ros::NodeHandle nh;
    string device_sn;
    int serial_nb;
    std::ostringstream oss;
    string default_sn("1205220158");
    if (nh.hasParam("kinect_front/serial_nb"))
    {
        ros::param::get("kinect_front/serial_nb", serial_nb);
        oss << serial_nb;
        device_sn = oss.str();
        std::cout<<"Using serial number : ("<<serial_nb<<")"<<std::endl;
    }else{
        std::cout<<"No serial number provided, using meka's default kinect front serial id : ("<<default_sn<<")"<<std::endl;
        device_sn = default_sn;
    }
    //char kinect_front_serial_nb[] = "1205220158";
    string configFilename = ros::package::getPath("m3openni_tracker") + "/openni_tracker.xml";
    ...
    XN_IS_STATUS_OK(nRetVal);
        ...
        for (xn::NodeInfoList::Iterator it = list.Begin(); it != list.End(); ++it, ++i)
        {
            ...
            if (deviceNode.IsValid() && deviceNode.IsCapabilitySupported(XN_CAPABILITY_DEVICE_IDENTIFICATION))
            {
                ...
                if ( string(strSerialNumber) == device_sn )
                {
                    printf("Choosing device [%d] %s (%s)\n", i, strDeviceName, strSerialNumber);
                    break;
                }
            }
            else
        ...
        int chosen = i;
        int nRetval = chosen;//scanf("%d", &chosen);
...
//                    printf("[%d] %s (%s)\n", i_depth, strDeviceName, strSerialNumber);
//                }
//                else
//                {
//                    printf("[%d] %s\n", i_depth, depthNodeInfo.GetCreationInfo());
//                }
...
//        printf("Choose device to open (1): ");
        ...
        int chosen_depth = 1;
        int nRetval_depth = 1;//scanf("%d", &chosen);
    ...
    ros::Rate r(30);
        ...
        ros::NodeHandle pnh("~");
        string frame_id("/kinect_front_link");
